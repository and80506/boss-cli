...
async def _dispatch_login(client: httpx.AsyncClient, qr_id: str) -> Credential:
    ...
    # obtained via the QR HTTP flow.  Try to supplement from browser cookies.
    if not credential.has_required_cookies:
        missing = credential.missing_required_cookies
        logger.info("QR login missing cookies: %s — trying browser supplement", ", ".join(missing))
        try:
            browser_cred = extract_browser_credential()
            if browser_cred:
                for name in missing:
                    if name in browser_cred.cookies:
                        cookies[name] = browser_cred.cookies[name]
                credential = Credential(cookies=cookies)
        except Exception as exc:
            logger.debug("Browser cookie supplement failed: %s", exc)

    if not credential.has_required_cookies:
        missing = credential.missing_required_cookies
        # If only __zp_stoken__ is missing, save anyway — most APIs still
        # work and SessionExpiredError gives actionable re-login guidance.
